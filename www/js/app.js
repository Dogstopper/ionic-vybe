var vybeApp = angular.module('vybe', ['ionic', 'ngCordova']);

vybeApp.run(function($ionicPlatform) {
...
vybeApp.controller('ContactsController', ['$scope', 'ContactsRetriever', '$localstorage',
                                          function($scope, ContactsRetriever, $localstorage) {
    $scope.contacts = $localstorage.getObject('contact_list');
    ContactsRetriever.getContacts().then(function(result) {
        $scope.contacts = result;
        $localstorage.setObject('contact_list',result);
        console.log("contacts: " + $scope.contacts.length);
    });

    $scope.getFakeContacts = function() {
        return ContactsRetriever.getFakeContacts();
    }
...
vybeApp.factory('ContactsRetriever', ['$cordovaContacts', '$q', function($cordovaContacts, $q) {
    var service = {};

    // Filters out contacts that do not have at least one valid phone number
    function containsValidPhoneNumber(element) {
        if (element.phoneNumbers != null) {
            if (element.phoneNumbers.length > 0) {
                console.log(JSON.stringify(element.phoneNumbers));
                var number;
                for (number in element.phoneNumbers) {
                    console.log(JSON.stringify(element.phoneNumbers[number]));
                    if (element.phoneNumbers[number] != null) {
                        if (element.phoneNumbers[number]['value'].length >= 7) {
                            return true;
                        }
                    }
                }
    ...
    function sortContact(a, b) {
        return a.toLowerCase().name.formatted < b.toLowerCase().name.formatted;
    }

    // Uses a deferal to find all the contacts using $cordovaContacts
    var getAllContacts = function() {
        var options = {};
        options.filter = "";
        options.multiple = true;
        options.fields = ['name', 'desiredName','phoneNumbers'];

        var deferred = $q.defer();
        $cordovaContacts.find(options).then(function(result) {
            deferred.resolve(result.filter(containsValidPhoneNumber))
        }, function(error) {
...
vybeApp.factory('$localstorage', ['$window', function($window) {
    return {
...
ionic.Platform.ready(function() {
    angular.bootstrap(document, ['vybe']);
});
